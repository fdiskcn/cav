...
void WidgetOvrtxView::ensureViewWindow()
{
    ...
    try {
        ...
        // Virtual OCCT windows still create a GLX/WGL context. Headless/Xvfb
        // setups often cannot satisfy that; RTX pixels do not depend on it.
        m_occWindowFailed = true;
        const QString occErr = QString::fromUtf8(TKernelUtils::errorMessage(err));
        if (m_status.isEmpty()) {
            m_status = occErr.isEmpty()
                ? tr("OCCT offscreen view is unavailable in this display.")
                : occErr;
        }
    }
}
...
void WidgetOvrtxView::redraw()
{
    try {
        ...
        m_status = QString::fromUtf8(TKernelUtils::errorMessage(err));
        this->update();
    }
}
